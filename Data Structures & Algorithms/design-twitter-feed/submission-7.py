class Twitter:
    ...
    def getNewsFeed(self, userId: int) -> List[int]:
        ...
        TC:
            first for loop
            F followers, the heap has max of F posts, use heappush, so
            log (F * log F)

            while loop, max 10 times, each time heappop & heappush takes O(log F)
            so O(10 * 2 log F) = O(log F)
            overall: log(F * log F + log F) = log(F * log F)
        SC: heap O(F), res O(10) -> O(1), so O(F)
        """
        heap = []
